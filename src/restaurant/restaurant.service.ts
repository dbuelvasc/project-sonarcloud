...
export class RestaurantService {
    ...
    async update(id: string, restaurant: RestaurantEntity): Promise<RestaurantEntity> {
        const PersitedRestaurant: RestaurantEntity = await this.restaurantRepository.findOne({ where: { id } });
        if (!PersitedRestaurant) {
            throw new BusinessLogicException("The restaurant with the given id was not found", BusinessError.NOT_FOUND);
        }
        return await this.restaurantRepository.save({ ...PersitedRestaurant, ...restaurant });
    }
